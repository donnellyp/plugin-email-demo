...
export default class EmailDemoPlugin extends FlexPlugin {
  ...
  init(flex, manager) {
    ...
    flex.MessageInput.Content.replace(<CustomMessageInput key="custom-message-input" />, {
      if: props => {
        ...
        try {
          ...
          if (isEmail) {
            // Sorry FE team, except Hando -- you deserve this
    ...
    // Change to e-mail related verbiage
    // doesn't have task contxt... manager.strings.TaskTabAgentChatLabel = 'Chat 1{{task}}';
    manager.events.addListener('selectedViewChanged', view => {
   ...
   * @param manager { import('@twilio/flex-ui').Manager }
   */
  registerEmailChannel(flex, manager) {
